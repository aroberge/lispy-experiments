...
(set-docstring not "Usage: (not x) ==>   #t <--> #f")
...
(set-docstring null? "Usage (null? x) ==>  true x is the empty list '() ")
...
(set-docstring and "Usage (and arg1 arg2 ...) ==>  true if all args are true ")
...
(set-docstring or "Usage (or arg1 arg2 ...) ==>  true if at least one arg is true ")

(define last (lambda (y) (cond ( (null? (cdr y)) (car y)) (else (last (cdr y))))))
(set-docstring last "Usage (list a b ...) ==>  (a b ...) ")

(define append (lambda (x y)
    (cond ( (null? x) y)
        (else (cons (car x) (append (cdr x) y))))))
(define list (lambda (x . y) (append (cons x '()) y)))
(set-docstring list "Usage (append '(a ...) '(b ...)) ==>  (a ... b ...) ")
